...
> {
   ...
   * @param args The arguments identifying the entity.
   */
  public async fetch(...args: Args): Promise<Value> {
    return (await this.get(...args)) ?? this.refresh(...args);
  }

  /**
   * Fetches an entity from the API, bypassing and then updating the cache.
   *
   * @param args The arguments identifying the entity.
   */
  public async refresh(...args: Args): Promise<Value> {
    const raw = await this.fetchRaw(...args);
